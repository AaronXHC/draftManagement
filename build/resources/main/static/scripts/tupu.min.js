...
document.addEventListener(visibilityChange, function() {
	"visible" == document[state] ? _isNeedReload && ($("#MainCy").html(""), $("#TrTxt").removeClass("active"), getData(_currentKeyNo)) : _isGraphLoaded || (_isNeedReload = !0)
}, !1);
var cy, id, activeNode, _rootData, _rootNode, _COLOR = {
	...
	_currentKeyNo, _companyRadius = 35,
	_personRadius = 15,
	_circleMargin = 10,
	_circleBorder = 3,
	_layoutNode = {},
...
function uniqeByKeys(c, b) {
	for (var a = [], d = {}, e = 0, g = c.length; e < g; e++) {
		var f;
		f = c[e];
		for (var h = b, k = h.length, l = []; k--;) l.push(f[h[k]]);
		f = l.join("|");
		f in d || (d[f] = !0, a.push(c[e]))
	}
	return a
}
Array.prototype.unique = function() {
	for (var c = [], b = {}, a = 0; a < this.length; a++) b[this[a]] || (c.push(this[a]), b[this[a]] = 1);
	return c
};
...
function getRootData(c) {
	for (var b = {
		nodes: [],
		links: []
	}, a = 0; a < c.length; a++) for (var d = c[a].graph.nodes, e = 0; e < d.length; e++) {
		var g = d[e],
			f = {};
		f.nodeId = g.id;
		f.data = {};
	...
	for (a = 0; a < c.length; a++) for (d = c[a].graph.relationships, e = 0; e < d.length; e++) g = d[e], f = {
		data: {}
	}, f.data.obj = g, f.data.showStatus = null, f.sourceNode = getGraphNode(g.startNode, b.nodes), f.targetNode = getGraphNode(g.endNode, b.nodes), f.linkId = g.id, f.source = getNodesIndex(g.startNode, b.nodes), f.target = getNodesIndex(g.endNode, b.nodes), b.links.push(f);
...
//			var f = b[e];
//			a != f.sourceNode.nodeId || f.targetNode.layout.level ? a != f.targetNode.nodeId || f.sourceNode.layout.level || (f.sourceNode.layout.level = d, c.push(f.sourceNode)) : (f.targetNode.layout.level = d, c.push(f.targetNode))
//		}
...
function setCategoryColor(c, b) {
	for (var a = 0; a < b.length; a++) b[a].sameLink = {
		length: 0,
		currentIndex: 0,
		isSetedSameLink: !1
	};
	for (a = 0; a < b.length; a++) {
		...
		if (0 == d.sameLink.isSetedSameLink) {
			...
			for (var h = 0; h < b.length; h++) {
				var k = b[h];
				d.linkId != k.linkId && !k.sameLink.isSetedSameLink && (k.sourceNode.nodeId == e && k.targetNode.nodeId == g || k.sourceNode.nodeId == g && k.targetNode.nodeId == e) && (f.push(k), k.sameLink.isSetedSameLink = !0)
			}
			for (d = 0; d < f.length; d++) e = f[d], e.sameLink.length = f.length, e.sameLink.currentIndex = d
		}
	}
	for (a = 0; a < c.length; a++) b = c[a], _currentKeyNo == b.data.obj.properties.keyNo ? (b.data.color = _COLOR.node.current, b.data.strokeColor = _COLOR.border.current) : "Company" == b.data.obj.labels[0] ? (b.data.color = _COLOR.node.company, b.data.strokeColor = _COLOR.border.company) : (b.data.color = _COLOR.node.person, b.data.strokeColor = _COLOR.border.person)
}
...
function getNodesIndex(c, b) {
	for (var a = 0, d = 0; d < b.length; d++) if (c == b[d].nodeId) {
		a = d;
		break
	}
	return a
...
function isNodeExist(c, b) {
	for (var a = !1, d = 0; d < b.length; d++) if (c == b[d].nodeId) {
		a = !0;
		break
	}
	return a
...
function filterLinksByNodes(c, b) {
	function a(a, b) {
		for (var c = !1, d = 0; d < a.length; d++) if (a[d].nodeId == b) {
			c = !0;
			break
		}
		return c
...
function getGraphNode(c, b) {
	for (var a = null, d = 0; d < b.length; d++) if (b[d].nodeId == c) {
		a = b[d];
		break
	}
	return a
...
function filterNodesByStockNum(c, b) {
	for (var a = [], d = 0; d < b.length; d++) c == b[d].data.obj.properties.stockPercent && a.push(b[d]);
	return a
}
...
function filterNodesByInvest(c, b, a) {
	...
	case "stockholder":
		...
		for (f = 0; f < b.length; f++) c = c.concat(d(b[f].nodeId, a));
		f = b.concat(c);
		break;
	case "legal":
		c = [];
...
function filter(c) {
	var b = [];
	(function(a) {
		for (var b = [], c = 0; c < a.nodes.length; c++) b.push(a.nodes[c]);
		for (var g = [], c = 0; c < a.links.length; c++) g.push(a.links[c]);
		...
		if (c && 0 != c) {
			...
			for (c = 0; c < g.length; c++) f.push(g[c].sourceNode), f.push(g[c].targetNode);
...
function resizeScreen() {
	isFullScreen() ? ($("#TrFullScreen").addClass("active"), $("#TrFullScreen").html('<span class="screen2ed"></span>退出')) : ($("#TrFullScreen").removeClass("active"), $("#TrFullScreen").html('<span class="screen2"></span>全屏'))
}
...
function toggleText() {
	$("#TrTxt").hasClass("active") ? ($("#TrTxt").removeClass("active"), cy.collection("edge").removeClass("edgeShowText")) : ($("#TrTxt").addClass("active"), cy.collection("edge").addClass("edgeShowText"))
}
...
function drawGraph(c) {
	...
	cy = cytoscape({
		...
		layout: {
			...
			stop: function(a) {
				...
				setTimeout(function() {
					"hidden" == document[state] ? (_isGraphLoaded = !1, console.log("stop _isGraphLoaded=false")) : _isGraphLoaded = !0
				}, 1E3)
			}
		},
		style: [{
			...
			style: {
				shape: "ellipse",
				width: function(a) {
					return "Company" == a.data("type") ? 160 : 135
				},
				height: function(a) {
					return "Company" == a.data("type") ? 160 : 135
				},
				...
				color: "#fff",
				padding: function(a) {
					return "Company" == a.data("type") ? 0 : 0
				},
				"font-size": 22,
				...
				"text-margin-y": function(a) {
					return "Company" == a.data("type") ?-15 : 2
				}
			}
		}, {
			selector: "edge",
	...
	cy.on("click", "node", function(a) {
		if (20 == a.target._private.style["z-index"].value) if (_isFocus = !0, a = a.target, highLight([a._private.data.id], cy), a.hasClass("nodeActive")) activeNode = null, $("#company-detail").hide(), a.removeClass("nodeActive"), cy.collection("edge").removeClass("edgeActive");
		else {
			var b = a._private.data;
			"Company" == b.type ? ($(".tp-detail").show(), showDetail(b.keyNo, "company_muhou3"), cy.collection("node").addClass("nodeDull")) : $(".tp-detail").hide();
			activeNode = a;
		...
		} else _isFocus = !1, activeNode = null, cy.collection("node").removeClass("nodeActive"), $(".tp-detail").fadeOut(), cancelHighLight()
	});
	var b = null;
	cy.on("mouseover", "node", function(a) {
		if (20 == a.target._private.style["z-index"].value) {
			...
			_isFocus || (cy.collection("edge").removeClass("edgeShow"), cy.collection("edge").removeClass("edgeActive"), c.neighborhood("edge").addClass("edgeActive"));
			clearTimeout(b);
			if (13 < c._private.data.name.length || c._private.data.keyNo && "p" == c._private.data.keyNo[0] && 3 < c._private.data.name.length) b = setTimeout(function() {
	...
	cy.on("mouseover", "edge", function(a) {
		_isFocus || (a = a.target, cy.collection("edge").removeClass("edgeActive"), a.addClass("edgeActive"))
	});
	cy.on("mouseout", "edge", function(a) {
		_isFocus || (a.target.removeClass("edgeActive"), activeNode && activeNode.neighborhood("edge").addClass("edgeActive"))
	});
	...
	cy.on("click", function(a) {
		a.target === cy && (_isFocus = !1, activeNode = null, cy.collection("node").removeClass("nodeActive"), $(".tp-detail").fadeOut(), cancelHighLight(), focusCancel(), filterReset())
	});
	cy.on("zoom", function() {.3 > cy.zoom() ? (cy.collection("node").addClass("hidetext"), cy.collection("edge").addClass("hidetext")) : (cy.collection("node").removeClass("hidetext"), cy.collection("edge").removeClass("hidetext"));
		setTimeout(function() {
	...
	cy.nodes().positions(function(a, b) {
		a._private.data.keyNo == _currentKeyNo && (b = cy.pan(), cy.pan({
...
function highLight(c, b) {
	...
	for (var a = 0; a < c.length; a++) {
		var d = c[a];
		b.nodes(function(a) {
			a._private.data.id == d && (a.removeClass("dull"), a.neighborhood("edge").removeClass("dull"), a.neighborhood("edge").addClass("edgeActive"), a.neighborhood("edge").connectedNodes().removeClass("dull"))
		})
	}
}
...
function highLightFilter(c, b) {
	...
	b.edges(function(b) {
		...
		a(c.target) && a(c.source) && (b.removeClass("dull"), b.addClass("edgeActive"))
	})
}
...
function getD3Position(c) {
	...
	(function(a) {
		function c(a, b) {
			for (var c = 0, d = 0; d < b.length; d++) if (a == b[d].nodeId) {
				c = d;
				break
			}
			return c
		...
		for (d = 0; d < a.links.length; d++) e = a.links[d], e.source = c(e.sourceNode.nodeId, a.nodes), e.target = c(e.targetNode.nodeId, a.nodes), e.index = d;
		a.layoutLinks = b(a);
	...
	var a = $("#MainD3 svg").width(),
		...
		h = 130,
		k = 35;
	50 > c.nodes.length ? (e = -800, g = 400) : 50 < c.nodes.length && 100 > c.nodes.length ? (e = -800, g = 350, h = 130, k = 35) : 100 < c.nodes.length && 150 > c.nodes.length ? (e = -900, g = 450) : 150 < c.nodes.length && 200 > c.nodes.length ? (e = -1E3, g = 500) : 200 < c.nodes.length && (e = -1600, g = 500, f = .6, h = 100, k = 35);
	50 < _maxChildrenLength && 100 > _maxChildrenLength ? (e = -2E3, g = 500) : 1E3 < _maxChildrenLength && 2E3 > _maxChildrenLength && (e = -4E3, g = 1500);
	console.log(c);
...
function downloadimgIE(c) {
	...
	for (var d in b) c = document.createElement("textarea"), c.name = d, c.value = b[d], a.appendChild(c);
...
function canvasImg(c) {
	...
	b.onload = function(a) {
		...
		c.drawImage(b, 0, 0);
		400 < a.width && (c.font = "28px 微软雅黑", c.fillStyle = "#aaaaaa", c.fillText("关联图谱由企查查基于公开信息利用大数据分析引擎独家生成", a.width / 2 - c.measureText("关联图谱由企查查基于公开信息利用大数据分析引擎独家生成").width / 2, a.height - 30));
		var e = new Image;
...
function getData(c, b) {
	...
	$.ajax({
		...
		dataType: "JSON",
		success: function(a) {
			(a = a.success) && void 0 != a.results && a.results[0] && a.results[0].data.length && 0 != a.results[0].data[0].graph.nodes.length ? ($(".printLogo").show(), $(".tp-foot").show(), $("#Main").show(), $("#no_data").hide(), _rootData = getRootData(a.results[0].data), domUpdate(_rootData)) : ($("#load_data").hide(), $(".printLogo").hide(), $(".tp-foot").hide(), $("#Main").hide(), $("#no_data").show())
		},
		error: function(a) {
			$("#load_data").hide();
			$(".printLogo").hide();
...
$(document).ready(function() {
	...
	$("#FocusBt").click(function() {
		var b = $("#FocusBt").text();
		if (!$(this).hasClass("focusDisable")) if ("聚焦" == b) if ($("#FocusInput").val()) {
			if (b = $("#FocusInput").attr("node_id")) $("#FocusBt").text("取消"), highLight([b], cy)
		} else faldia({
			content: "请点击选取结点"
		});
		else "取消" == b && focusCancel()
	});
